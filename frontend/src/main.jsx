...
function QuestionInput({ q, value, parts, choose, revealed }) {
  ...
  if (q.question_type === 'order') return <div className="mt-6 space-y-3"><p className="text-sm text-slate-400">Ordering control: click choices in the desired sequence. Current: {value || 'none'}</p>{q.choices.map(c => <button key={c.key} onClick={() => !parts.includes(c.key) && choose([...parts, c.key].join(','))} className={`w-full rounded-2xl border p-4 text-left ${classify(c.key)}`}><b>{c.key}.</b> {c.text}</button>)}<button className="btn" onClick={() => choose('')}>Clear order</button></div>
  if (q.question_type === 'match') return <div className="mt-6 space-y-3"><p className="text-sm text-slate-400">Matching is represented as paired answer tokens for scoring. Select the best provided pair sequence.</p>{q.choices.map(c => <button key={c.key} onClick={() => choose(c.key)} className={`w-full rounded-2xl border p-4 text-left ${classify(c.key)}`}><b>{c.key}.</b> {c.text}</button>)}</div>
